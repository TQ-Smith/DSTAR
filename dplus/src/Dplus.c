...
void bootstrap(BlockList_t* blockList, int replicates, bool standard) {
    ...
    if (standard) {
        ...
        int dNumGreater = 0, dplusNumGreater = 0;
        for (int i = 0; i < replicates; i++) {
            if (fabs(blockList -> dNum / blockList -> dDenom) > fabs(dDis[i]))
                dNumGreater++;
            if (fabs(blockList -> dplusNum / blockList -> dplusDenom) > fabs(dplusDis[i]))
                dplusNumGreater++;
        }
        blockList -> dP = dNumGreater / (double) replicates;
        blockList -> dplusP = dplusNumGreater / (double) replicates;
    }
