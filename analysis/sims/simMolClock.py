...
# Added to adjust molecular clock.
parser.add_argument("--scale", type=float,help="Lambda to adjust molecular clock.")
parser.add_argument("--pop", type=int, help="The population to rescale.")
...
# Scale all the divergence times.
t_1234 = args.t_1234 + int((args.scale - 1) * args.t_12)
t_123 = args.t_123 + int((args.scale - 1) * args.t_12)
t_12 = args.t_12 + int((args.scale - 1) * args.t_12)
t_gf = args.t_gf + int((args.scale - 1) * args.t_12)
sampling_time = int((args.scale - 1) * args.t_12)

demography = msprime.Demography()
demography.add_population(name="P1",initial_size=args.Ne)
demography.add_population(name="P2",initial_size=args.Ne)
demography.add_population(name="P3",initial_size=args.Ne)
demography.add_population(name="P4",initial_size=args.Ne)
demography.add_population(name="P12",initial_size=args.Ne)
demography.add_population(name="P123",initial_size=args.Ne)
demography.add_population(name="P1234",initial_size=args.Ne)

if args.f>0:
  demography.add_mass_migration(
    time=t_gf, source = args.recipient_population, dest=args.donor_population,proportion = float(args.f/100))

demography.add_population_split(
	time=t_12, derived=['P1', 'P2'], ancestral='P12',
)

demography.add_population_split(
        time=t_123, derived=['P12', 'P3'], ancestral='P123',
)

demography.add_population_split(
        time=t_1234, derived=['P123', 'P4'], ancestral='P1234',
)

if args.pop == 1:
	samples = [
		msprime.SampleSet(args.p1_sample_size, ploidy=1, population='P1', time=0),
		msprime.SampleSet(args.p2_sample_size, ploidy=1, population='P2', time=sampling_time),
		msprime.SampleSet(args.p3_sample_size, ploidy=1, population='P3', time=sampling_time),
		msprime.SampleSet(args.p4_sample_size, ploidy=1, population='P4', time=sampling_time),
	]
elif args.pop == 2:
	samples = [
		msprime.SampleSet(args.p1_sample_size, ploidy=1, population='P1', time=sampling_time),
		msprime.SampleSet(args.p2_sample_size, ploidy=1, population='P2', time=0),
		msprime.SampleSet(args.p3_sample_size, ploidy=1, population='P3', time=sampling_time),
		msprime.SampleSet(args.p4_sample_size, ploidy=1, population='P4', time=sampling_time),
	]
